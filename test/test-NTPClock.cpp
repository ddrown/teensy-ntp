...
void test_gettime() {
  ...
  TEST_ASSERT_EQUAL(1, clock.getTime(&sec, &fractional));
  TEST_ASSERT_EQUAL(12347678, sec.v);
  // 999999/1000000*2^32 = 4294963001
  TEST_ASSERT_UINT32_WITHIN(1, 4294963001, fractional);

  When(Method(ArduinoFake(), micros)).Return(4000999999);
  TEST_ASSERT_EQUAL(1, clock.getTime(&sec, &fractional));
  TEST_ASSERT_EQUAL(12349678, sec.v);
  // 999999/1000000*2^32 = 4294963001
  TEST_ASSERT_UINT32_WITHIN(1, 4294963001, fractional);
}
...
void test_wraptime() {
  ...
  clock.setTime(4293000000, TaiNtpTime(12349971));

  When(Method(ArduinoFake(), micros)).Return(32703); // 2^32 wrap by 1s
  TEST_ASSERT_EQUAL(1, clock.getTime(&sec, &fractional));
  TEST_ASSERT_EQUAL(12349972, sec.v);
...
void test_setppb() {
  ...
  TEST_ASSERT_EQUAL(12347676, sec.v);
...
void test_realvalues() {
  ...
  clock.setPpb(-668);
  clock.setTime(838698, TaiNtpTime(3785790043));
  ...
  TEST_ASSERT_EQUAL(3785790163, sec.v);
  ...
  TEST_ASSERT_EQUAL(3785790164, sec.v);
  ...
  TEST_ASSERT_EQUAL(3785790263, sec.v);
  ...
  TEST_ASSERT_EQUAL(3785790264, sec.v);
...
void test_getoffset() {
  ...
  TEST_ASSERT_EQUAL(-42949689453LL, clock.getOffset(120838782, TaiNtpTime(3785790153), 0));
  TEST_ASSERT_EQUAL(-16493, clock.getOffset(120838782, TaiNtpTime(3785790163), 0));
  TEST_ASSERT_EQUAL(-13624, clock.getOffset(121838782, TaiNtpTime(3785790164), 0));
  TEST_ASSERT_EQUAL(-47416, clock.getOffset(220838856, TaiNtpTime(3785790263), 0));
  TEST_ASSERT_EQUAL(-44547, clock.getOffset(221838856, TaiNtpTime(3785790264), 0));
}
...
void test_backwards() {
  ...
  clock.setPpb(-975);
  clock.setTime(545545804, TaiNtpTime(3785904938));

  TEST_ASSERT_EQUAL(1, clock.getTime(546546017, &sec, &fractional));
  TEST_ASSERT_EQUAL(3785904939, sec.v);
