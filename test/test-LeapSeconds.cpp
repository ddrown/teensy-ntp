...
void test_table_timestamps_agree_with_datetime() {
  ...
  TEST_ASSERT_EQUAL(leapSeconds[leapSecondsCount - 1].effectiveNtpTime, taiToWireNtp(d2017.ntptime()));
}
...
void test_offset_at_tai_boundary_is_new_value_not_leap_instant() {
  bool isLeapInstant;
  TEST_ASSERT_EQUAL(37, leapSecondOffsetAtTai(3692217637UL, &isLeapInstant));
...
void test_offset_one_before_tai_boundary_is_leap_instant() {
  bool isLeapInstant;
  TEST_ASSERT_EQUAL(36, leapSecondOffsetAtTai(3692217637UL - 1, &isLeapInstant));
  TEST_ASSERT_TRUE(isLeapInstant);
}

void test_offset_two_before_tai_boundary_is_old_value_not_leap_instant() {
  bool isLeapInstant;
  TEST_ASSERT_EQUAL(36, leapSecondOffsetAtTai(3692217637UL - 2, &isLeapInstant));
  TEST_ASSERT_FALSE(isLeapInstant);
}

void test_offset_at_tai_holds_forever_after_last_entry() {
  bool isLeapInstant;
  TEST_ASSERT_EQUAL(37, leapSecondOffsetAtTai(3692217637UL + 100000000UL, &isLeapInstant));
  TEST_ASSERT_FALSE(isLeapInstant);
}

void test_tai_to_wire_ntp_normal_second() {
  // Well clear of any boundary: 2020-06-15ish, offset 37 throughout.
  TEST_ASSERT_EQUAL(3800000000UL, taiToWireNtp(3800000000UL + 37));
}
...
void test_tai_to_wire_ntp_leap_instant_and_next_day_collide_on_wire() {
  TEST_ASSERT_EQUAL(3692217600UL, taiToWireNtp(3692217637UL - 1)); // leap instant
  TEST_ASSERT_EQUAL(3692217600UL, taiToWireNtp(3692217637UL));     // next day :00
}
